...
export const inventoryService = {
  ...
  getAll: async (): Promise<InventoryTransaction[]> => {
    const response = await api.get<InventoryTransaction[]>('/api/inventory');
    return response.data;
  },

  // 入庫登録
  stockIn: async (data: InventoryTransactionRequest): Promise<InventoryTransaction> => {
    const response = await api.post<InventoryTransaction>('/api/inventory/in', data);
    return response.data;
  },

  // 出庫登録
  stockOut: async (data: InventoryTransactionRequest): Promise<InventoryTransaction> => {
    const response = await api.post<InventoryTransaction>('/api/inventory/out', data);
    return response.data;
  },

  // 補給品別トランザクション取得
  getBySupplyId: async (supplyId: number): Promise<InventoryTransaction[]> => {
    const response = await api.get<InventoryTransaction[]>(`/api/inventory/supply/${supplyId}`);
    return response.data;
  },
};
